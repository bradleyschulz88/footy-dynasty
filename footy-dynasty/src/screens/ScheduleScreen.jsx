...
import { TRAINING_INFO, formatDate, formatMonth, daysInMonth, startOfMonth, getDayOfWeek, prevMonth, nextMonth } from "../lib/calendar.js";
  ...
  function evDot(ev) {
    ...
    if (ev.type === "round") {
      ...
      return {
        color: "var(--A-accent)",
        icon: "🏉",
        label: opp ? `Rd ${ev.round} vs ${opp.short}` : `Rd ${ev.round}`,
      };
    }
    ...
    <div className="anim-in space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setViewDate(prevMonth(viewDate))} className={css.btnGhost + " px-3 py-2"}>
            <ChevronLeft className="w-4 h-4" />
          </button>
          <span className="font-display text-xl tracking-wide text-atext min-w-[180px] text-center">{formatMonth(viewDate)}</span>
          <button type="button" onClick={() => setViewDate(nextMonth(viewDate))} className={css.btnGhost + " px-3 py-2"}>
        ...
        <div className={`${css.panel} lg:col-span-2 p-2 md:p-4 overflow-x-auto`}>
          <div className="min-w-[420px]">
            <div className="grid grid-cols-7 mb-2">
              {dayNames.map((d) => (
                <div key={d} className="text-center text-[11px] font-bold uppercase tracking-widest text-atext-mute py-1">
                  {d}
                </div>
              ))}
            </div>
              ...
              {cells.map((dateStr, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: isToday ? "rgba(0,224,255,0.10)" : isPast && dayEvs.length ? "var(--A-panel)" : "var(--A-panel-2)",
                      border: "1px solid var(--A-line)",
                    }}
                  >
                    <div
                      className={`text-xs font-bold mb-1 ${isToday ? "text-aaccent" : isPast ? "text-atext-mute" : "text-atext-mute"}`}
                    >
                      {dateStr.slice(8)}
                    </div>
                    <div className="flex flex-col gap-0.5">
                      {dayEvs.slice(0, 3).map((ev, ei) => {
                        ...
                        return (
                          <div
                            key={dk}
                            className="rounded text-[10px] font-bold px-1 py-0.5 truncate leading-tight"
                            style={{
